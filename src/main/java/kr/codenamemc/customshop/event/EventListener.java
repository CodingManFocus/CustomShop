package kr.codenamemc.customshop.event;

import kr.codenamemc.customshop.handler.ItemHandler;
...
public class EventListener implements Listener {
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (event.getClick().isLeftClick() && clickedInventory != null && clickedInventory.getHolder() == null) {
            itemHandler.buyItem(player, value, slot); // 상점 이름과 슬롯으로 구매 처리
            event.setCancelled(true);
        } else if (event.getClick().isRightClick() && clickedInventory != null && clickedInventory.getHolder() == null) {
            itemHandler.sellItem(player, value, slot); // 상점 이름과 슬롯으로 판매 처리
            event.setCancelled(true);
        }
    }
    @EventHandler
